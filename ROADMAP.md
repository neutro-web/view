...
| Control flow (`<For>`/`<Index>`/`<Show>`/`<Switch>`) | **Mostly have** | `list` (keyed, by-identity) ≈ `<For>`; `conditional` ≈ `<Show>`; `recycle` (positional, non-keyed) ≈ `<Index>` (**landed** `3b00064`, 2026-06-29). `<Switch>`/multi-branch remains the one **gap**. |
