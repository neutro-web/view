...
export class EqualityPolicyInferencer {
    ...
    constructor(config) {
        ...
        this.tsLibDir = normPath(path.dirname(ts.getDefaultLibFilePath({ target: ts.ScriptTarget.ES2022 })));
    }
    /**
     ...
     */
    inferCall(call, checker) {
        if (!this.isSignalOrDerivedCall(call, checker))
            return null;
     ...
     */
    hasExplicitEquals(call, checker) {
        const optionsArg = call.arguments[1];
        if (!optionsArg)
            return false;
        // Layer 1: direct object literal — AST-based, covers all three literal forms
        if (ts.isObjectLiteralExpression(optionsArg)) {
            return optionsArg.properties.some((p) => (ts.isPropertyAssignment(p) && ts.isIdentifier(p.name) && p.name.text === 'equals') ||
        ...
        if (optionsType.flags & ts.TypeFlags.Any)
            return true;
     ...
     */
    isSignalOrDerivedCall(call, checker) {
        ...
        if (!nameNode)
            return false;
        if (nameNode.text !== 'signal' && nameNode.text !== 'derived')
            return false;
    ...
    categorize(type, checker) {
        ...
        if (this.isPrimitiveType(type))
            return 'OBJECT_IS';
        if (this.isMutableStandardContainer(type, checker))
            return 'FALSE';
...
//# sourceMappingURL=equality-policy-inference.js.map