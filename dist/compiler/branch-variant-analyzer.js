...
import { isLiteralKeyExpr, isNvSignalType, normPath, signalSymbolId, symbolIsFromNvCore, } from './signal-type-utils';
const EMPTY = { kind: 'SIGNALS', signals: new Set() };
...
export class BranchVariantAnalyzer {
    ...
    analyzeCall(call, checker) {
        if (!this.isDerivedOrEffectCall(call, checker))
            return null;
     ...
     */
    analyzeBody(body, checker) {
        if (ts.isBlock(body))
            return this.analyzeBlock(body, checker);
        return this.analyzeExpr(body, checker);
    }
    // ── Nominal identification ─────────────────────────────────────────────────
    isDerivedOrEffectCall(call, checker) {
        ...
        if (!nameNode)
            return false;
        if (nameNode.text !== 'derived' && nameNode.text !== 'effect')
            return false;
    ...
    extractComputeBody(call, checker) {
    ...
    analyzeBlock(block, checker) {
        ...
        for (const stmt of block.statements) {
            if (ts.isVariableStatement(stmt)) {
                ...
                returnCount++;
                if (returnCount > 1)
                    return decline('multiple return statements in block body');
                if (stmt.expression) {
    ...
    analyzeExpr(rawExpr, checker) {
        ...
        if (ts.isCallExpression(expr))
            return this.analyzeCallExpr(expr, checker);
        // ── ConditionalExpression: union all three parts ───────────────────────────
        if (ts.isConditionalExpression(expr)) {
            return this.merge(this.analyzeExpr(expr.condition, checker), this.merge(this.analyzeExpr(expr.whenTrue, checker), this.analyzeExpr(expr.whenFalse, checker)));
        }
        ...
        if (ts.isPrefixUnaryExpression(expr)) {
            return this.analyzeExpr(expr.operand, checker);
        }
        ...
        if (ts.isElementAccessExpression(expr)) {
            return this.merge(this.analyzeExpr(expr.expression, checker), this.analyzeExpr(expr.argumentExpression, checker));
        }
    ...
    analyzeCallExpr(call, checker) {
        ...
        if (this.isUntrackedCall(call, checker))
            return EMPTY;
    ...
    resolveReadId(callee, checker) {
        ...
        if (ts.isIdentifier(callee)) {
            ...
            const key = callee.argumentExpression.text;
...
//# sourceMappingURL=branch-variant-analyzer.js.map