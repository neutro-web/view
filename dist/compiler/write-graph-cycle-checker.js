...
import { isLiteralKeyExpr, isNvSignalType, signalSymbolId, symbolIsFromNvCore, } from './signal-type-utils';
// ── Public API ─────────────────────────────────────────────────────────────────
export class WriteGraphCycleChecker {
     ...
     */
    check(program, verdicts) {
        const checker = program.getTypeChecker();
        const acceptVerdicts = verdicts.filter((v) => v.kind === 'ACCEPT');
        if (acceptVerdicts.length === 0)
            return [];
        ...
        for (const verdict of acceptVerdicts) {
            ...
            const reads = readResult.kind === 'UNKNOWN' ? new Set() : readResult.signals;
            syncEdges.push({
                reads,
     ...
     */
    analyzeSourceReads(sourceArg, checker) {
     ...
     */
    walkReads(node, checker, reads, incomplete) {
        ...
        if (ts.isCallExpression(node)) {
            ...
            if (this.isUntrackedCall(node, checker))
                return;
     ...
     */
    addSignalRead(callee, checker, reads, incomplete) {
        let sym;
        if (ts.isIdentifier(callee)) {
            ...
            const keyText = callee.argumentExpression.text;
    ...
    buildGraph(syncEdges) {
        ...
        const addEdge = (from, to, call) => {
            ...
            if (!attribution.has(from))
                attribution.set(from, new Map());
            const toMap = attribution.get(from);
            if (toMap !== undefined) {
                if (!toMap.has(to))
                    toMap.set(to, new Set());
     ...
     */
    detectCycles(graph, attribution) {
 ...
 */
function findInvolvedSyncs(cycle, attribution) {
    const syncs = new Set();
    for (let i = 0; i < cycle.length; i++) {
        ...
        if (callSet !== undefined) {
            for (const c of callSet)
                syncs.add(c);
        }
    }
    return [...syncs];
}
//# sourceMappingURL=write-graph-cycle-checker.js.map