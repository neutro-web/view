 ...
 */
export function symbolIsFromNvCore(sym, checker, nvCorePath) {
    let resolved = sym;
    for (let i = 0; i < 20; i++) {
 ...
 */
export function isNvSignalType(type, checker, nvCorePath) {
 ...
 */
export function getThunkReturnSignalType(type, checker, nvCorePath) {
    // If this type itself has .set, it IS a SignalAccessor (Path A), not a thunk
    if (type.getProperty('set'))
        return null;
    ...
    if (isNvSignalType(ret, checker, nvCorePath))
        return ret;
 ...
 */
export function signalSymbolId(sym, checker) {
    let resolved = sym;
 ...
 */
export function isLiteralKeyExpr(expr) {
    return ts.isStringLiteral(expr) || ts.isNumericLiteral(expr);
}
//# sourceMappingURL=signal-type-utils.js.map