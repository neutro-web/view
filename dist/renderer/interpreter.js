...
export function walkPath(root, path) {
    ...
    for (let depth = 0; depth < path.length; depth++) {
        ...
        if (child === undefined) {
            throw new Error(`[nv/interpreter] walkPath: no child at index ${idx} (depth ${depth}). ` +
 ...
 */
function wireBinding(binding, targetNode, doc) {
    switch (binding.kind) {
        case 'text': {
        ...
        case 'list':
        case 'sync': {
            throw new Error(`[nv/interpreter] v0: '${binding.kind}' binding is designed but not yet implemented in the interpreter. Deferred per IR §9.2.`);
        }
        default: {
            const _exhaustive = binding;
            throw new Error(`[nv/interpreter] Unknown binding kind: ${_exhaustive.kind}`);
        }
    }
}
// ── TextBinding ───────────────────────────────────────────────────────────────
function wireText(binding, commentNode, doc) {
...
// ── AttrBinding ───────────────────────────────────────────────────────────────
function wireAttr(binding, el) {
    if (el.nodeType !== 1 /* ELEMENT_NODE */) {
        throw new Error(`[nv/interpreter] AttrBinding expects an Element node; got nodeType ${el.nodeType}`);
    }
    const element = el;
...
// ── PropBinding ───────────────────────────────────────────────────────────────
function wireProp(binding, el) {
    if (el.nodeType !== 1 /* ELEMENT_NODE */) {
        throw new Error(`[nv/interpreter] PropBinding expects an Element node; got nodeType ${el.nodeType}`);
    }
    const element = el;
    const name = binding.name;
    effect(() => {
        ;
        element[name] = binding.expr();
    });
}
// ── EventBinding ──────────────────────────────────────────────────────────────
function wireEvent(binding, el) {
    if (el.nodeType !== 1 /* ELEMENT_NODE */) {
        throw new Error(`[nv/interpreter] EventBinding expects an Element node; got nodeType ${el.nodeType}`);
    }
    const element = el;
...
// ── ChildBinding ──────────────────────────────────────────────────────────────
function wireChild(binding, anchorNode, doc) {
    ...
    effect(() => {
        ...
        if (v !== null && v !== undefined && typeof v === 'object') {
            throw new Error(`[nv/interpreter] v0: ChildBinding received non-primitive value (${Object.prototype.toString.call(v)}). DOM Node / TemplateIR values are designed but not yet implemented. Expected: string | number | null | undefined.`);
        }
        textNode.data = v == null ? '' : String(v);
    });
}
// ── ConditionalBinding ────────────────────────────────────────────────────────
function wireConditional(binding, anchorNode, doc) {
    // Anchor is a Comment node; branches mount before it.
    const parent = anchorNode.parentNode;
    if (parent === null) {
        throw new Error('[nv/interpreter] ConditionalBinding: anchor has no parent');
    }
    let branchDisposer = null;
    effect(() => {
 ...
 */
function mountFragment(ir, parent, doc, before = null) {
    ...
    const targets = ir.bindings.map((b) => {
 ...
 */
export function mount(ir, parent, doc) {
