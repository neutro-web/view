...
function compareAttributes(a, b, path) {
    if (a.attributes.length !== b.attributes.length) {
        const namesA = attrNames(a);
        const namesB = attrNames(b);
        return {
            equal: false,
            diffPath: `${path} <${a.tagName.toLowerCase()}>: ` +
    ...
    for (let i = 0; i < a.attributes.length; i++) {
        ...
        if (attr.value !== bVal) {
            return {
                equal: false,
                diffPath: `${path} <${a.tagName.toLowerCase()}> @${attr.name}: ` + `"${attr.value}" vs "${bVal}"`,
            };
        }
    }
    return ok();
}
function compareChildren(a, b, path) {
