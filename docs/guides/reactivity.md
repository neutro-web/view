...
For the `.nv` template layer (where signal reads are erased to calls at compile time) see [Authoring .nv](/guides/authoring-nv). For full type signatures see [API Reference](/api/).
...
- In `.nv` files, bare identifiers that refer to signals are erased to calls (`count` becomes `count()`) by the compiler. This is a compile-time transform; the runtime behavior is identical to calling the accessor manually. See [Authoring .nv](/guides/authoring-nv).
