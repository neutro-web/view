...
- **Runtime correctness baseline:** §12 conformance suite passes **40/40** against
  contract v0.4.2 (23 original checklist items + §12.17a/b error-path cases + property
  fuzzer + §B2–B8 invariant/coverage tests + §12.24 owner-context 4-test addition
  [getOwner/runWithOwner: ownership redirection, no-redirect control, observation-
  neutrality, runWithOwner(null) detach]). Three implementation bugs surfaced and fixed
