...
**Decision:** `emitComponentFactory` reshaped to emit `ComponentRef` + `.mount` sugar
(A2 design, `factory-shape-convergence-design.md` v1.1). Back-ends (`interpreter.ts`,
`emitted-mount.ts`, `core.ts`) unchanged — verified by S-A2 spike + TC-C15 parity.
`createRoot`/`onCleanup` dropped from forced import list.
...
DOM identical). All failed before commit 9056f4c; all pass after.
