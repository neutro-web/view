...
- **Render-hole erasure unbuilt** — see seam above. Blocks runnable event handlers from `.nv`
  unless authors write explicit `.set()`. (Build-pipeline spec treats this as a first-class item.)
