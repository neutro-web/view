...
Last verified against source: **2026-07-03 (Follow-up B′ — `wireRecycledList` HWM pooling collapse).** Contract **v0.4.3**, Template IR **v0.4.4**.
...
| `emitted-mount.ts` | REAL | Compiler back-end. Consumes a **real-thunk** IR → specialized mount. Handles text/attr/prop/event/child/conditional/list/component/**slot-outlet**/**classlist**; **recycled-list** stub (throws); sync throws. **Not on the v1 build-pipeline path** (build uses interpreter `mount`). `classlist` case: per-key `effect` for ≤6 toggle entries, one looping `effect` for >6 (T=6 placeholder; `TODO(threshold)` comment). Component case: `capturedParentOwner = getOwner()` before `createRoot`; passes `childSlotContext` to recursive `emitSetup`. Slot-outlet case calls factory (`content(slotProps)`) and uses `runWithOwner(capturedParentOwner, () => createRoot(...))` — parent-lexical ownership (D-slot-1). **Invocation-scoped by construction** (same as interpreter — `capturedParentOwner = getOwner()` at L550 inside the row's `createRoot` when nested in a list). D-slot-2 CLOSED 2026-06-24. Direct-capture (`componentFactory` + `propEntries`), never captures binding object. |

### `src/renderer/` → `@neutro/view/renderer`
| File | Status | Notes |
|---|---|---|
| `ir.ts` | REAL | **`DeferredSwapBinding` added (PT-1b-i tier-2, 2026-07-08):** `{ kind:'deferred-swap', pathIndex, pending: ReactiveExpr<boolean>, branches, fallback }` — `SwitchBinding` shape plus `pending`; see `docs/gates/pt1b-i-deferred-swap.md`. 14 binding kinds now. IR types, matches Template-IR **v0.4.4**. 13 binding kinds (6 PoC + List + RecycledListBinding + ComponentBinding + SlotOutletBinding + ClassListBinding + **StyleVarBinding** landed; Sync deferred). `RecycledListBinding` = `{ kind:'recycled-list', pathIndex, items, itemTemplate }` (v0.4.4) — non-keyed, position-identity contract. `ClassListBinding` = `{ kind:'classlist', pathIndex, entries: readonly ClassListEntry[] }` where entry is `{ kind:'static'; token } \| { kind:'toggle'; key; expr }` — additive union member (v0.4.1). `StyleVarBinding` = `{ kind:'style-var', pathIndex, varName: string, expr }` (v0.4.2). `ComponentBinding` adds `component: ComponentRef`, `props: PropEntry[]`, `propNames`, `slots: SlotEntry[]`. `SlotEntry.content` is now a factory `(props: SlotProps) => TemplateIR` (hard-cut, v0.4). `SlotOutletBinding` = `{ kind:'slot-outlet', pathIndex, name, fallback?: TemplateIR, props?: readonly PropEntry[] }`. `TemplateIR` root carries optional `styleArtifact` ({staticCss, scopeHash, varBindingDescs?}) + `classRewrites` (v0.4.2, .nv-FE-only). `SlotContent` exported. All types local-structural (no DOM/core imports). |
| `interpreter.ts` | REAL | **`wireDeferredSwap` added (PT-1b-i tier-2, 2026-07-08, `717ce43`):** deferred-swap construct — holds the currently-revealed branch mounted+live while `binding.pending()` is true, swaps atomically (insert-new-then-dispose-old) once settled. Independently duplicates `wireSwitch`'s winner-selection loop by design (G0-protected, no shared helper — see gate doc Ruling 2's correction). Two real bugs found+fixed post-landing via Task 5's real-browser tests, not caught by design review: (1) the revealed subtree must be mounted via `runWithOwner(capturedOwner, ...)` with `capturedOwner = getOwner()` captured BEFORE the `effect()` call — mounting it from inside the effect's own compute (as `wireConditional`/`wireSwitch` safely do, since they always intend to rebuild) makes it a child of the effect, and `preRunCleanup` disposes all of a node's children at the start of every one of ITS OWN recomputes, destroying the held subtree on any re-run before the pending-check ever runs; (2) swap order must be insert-new-then-dispose-old, not the reverse (a real, `MutationObserver`-visible "neither attached" gap otherwise — though invisible to any actual paint). Full writeup: `docs/gates/pt1b-i-deferred-swap.md` (Findings N/O). Back-end / **semantic ground truth**. Exports `mount(ir, parent, doc): () => void` and `walkPath`. `mount` **creates its own `createRoot`**; effects enqueued, run on first flush. `mountFragment` internal. Handles all 6 PoC kinds + list + **recycled-list** (`wireRecycledList`) + component + slot-outlet + **classlist** (`wireClassList`); sync throws. **`wireRecycledList` reconciliation strategy changed (Follow-up B′, `ab68ee2`, 2026-07-03):** was dispose-based (pool sized to exact `windowN` every run, full teardown on shrink); is now high-water-mark **retention**-based — on shrink, rows are detached from the DOM and stop being fed via `.set()`, but are NOT disposed; on regrow, retained slots are reused before allocating new ones. `wireRecycledListHWM` (the Phase 1 prototype name) no longer exists — it was renamed to `wireRecycledList`, and the old dispose-based implementation was deleted, not kept as an alternate path. Pool never shrinks below its high-water-mark in the current build (**eviction/cap deferred — unbounded retention**, an open architecture question as of this writing, see `docs/decision-log.md` Follow-up B′ entries). Correctness of the retention strategy is coupled to the reactive core's demand-driven scheduling (an unwritten signal does no work) — see the doc comment at `wireRecycledList`'s definition for the exact dependency. `wireClassList`: statics added once at mount via `classList.add`; toggles wired as per-key `effect` for ≤6, one looping `effect` for >6 (T=6; `TODO(threshold)` comment). `wireSlotOutlet` calls `content(slotProps)` factory (v0.4). Slot content rendered via `runWithOwner(capturedParentOwner, () => createRoot(...))` — parent-lexical ownership (D-slot-1). **Slot-content ownership is invocation-scoped by construction:** `capturedParentOwner = getOwner()` at wire time (L680); inside an `each` row this is the row's `createRoot`, so slot content disposes with the row. D-slot-2 CLOSED 2026-06-24 (premise dissolved) — do not flip this. Nested roots bridged via `onCleanup`. |
| `html-tag.ts` | REAL | **`match()` gained an optional third `pending` arg (PT-1b-i, 2026-07-08):** presence forks the resulting `MatchSentinel`'s construction to `DeferredSwapBinding`; absence unchanged (`SwitchBinding`, byte-identical path). Tagged-template front-end (`createHtmlTag(doc)`). Handles `text`, `attr`, `prop` (`.name=`), `event` (`@name=`), `component`, **slot-outlet** (`${slots('name')}` sentinel), and **classlist** (`${classes(...)}` sentinel). `classes(...args)` returns a `ClassesSentinel` (`__nvClasses` brand); detected in `buildHtmlHoleBinding`'s attr branch when `name === 'class'`. **`cx(...args): string`** — pure string builder (no reactivity); handles string/object/array/falsy args recursively. A bare object literal at a `class` hole still throws (validator allowlist). Named slot capture, fallback sentinel, `slot(name, factory)` fill sentinel. Exports `slots`, `slot`, `cx`, `classes`, `ClassesSentinel` from renderer barrel. |
| `nv-parser.ts` | PARTIAL → **REAL for the build path** | **`<switch pending=>` LANDED (PT-1b-i, 2026-07-08, `26b9832`):** optional `pending=` attribute on `<switch>` (same extraction mechanism as `<match>`'s `when=`), forks the emitted binding to `DeferredSwapBinding`; absent, byte-identical `SwitchBinding` path. Zero-`<match>`-children validation unchanged either way. Note: `parseNvFileForEmit`'s codegen path (`nv-emitter.ts`) has no case for `'deferred-swap'` — compiling (not just interpreting) `<switch pending=>` throws loudly at codegen time via the existing `default:` branch, mirroring `emitted-mount.ts`'s `recycled-list`-style stub. `parseNvFile` (structural-only, stub thunks) + `parseNvFileForEmit` (real emit payload). **`<each>` authoring LANDED (2026-06-22):** both `walkNvNodeList` and `parseNvFileForEmit`'s thunk builder handle `<each .of=... key=... let={...}>` → `ListBinding`. Variant-A adapter at construction in both FEs. **`<each>`-in-slot + static-class structural collapse LANDED (Increment SS, 2026-06-23):** `pushListBinding` module-level helper shared by `processHtmlTemplate` and `buildNvSlotContentIR`; `walkNvNodeList` gains `diagnostics` param; `buildNvSlotContentIR` wires `lists: slotLists`. `liftStaticClassBindings(root: ParentNode, allPaths, bindings)` module-level helper strips `class=` attrs into `ClassListBinding` entries; called on `frag` (main) and `fragWrapper` (slot) before shape serialization; `patchClasslistTokens` component-case regex rewrite REMOVED. Main-path static-class live bug CLOSED. `html-tag.ts` `buildSlotContentIR` now wires `lists` from `walkNodeList`. **Classlist LANDED (Increment C, 2026-06-22):** `buildNvHoleBinding` attr branch routes `class` + object/array literal → `ClassListBinding`; computed/shorthand keys → `AttrBinding` fallback. Structural path emits `stubExpr` for toggle `expr` (structural-only — by design). `parseNvFileForEmit` classlist branch (~L2188–2257) extracts real `boolSrc: string` per toggle entry. **D-cl-3 (known defect):** key extraction uses `prop.name.getText()` at four sites (L367/L399/L2205/L2237); returns source text with surrounding quotes for string-literal keys (e.g. `'is-active'` → key `"'is-active'"`). Identifier keys unaffected. Fix deferred to S0 (`propertyKeyText` helper; `p.name.text` for Identifier/StringLiteral). `eraseSignalReadsInNode` PropertyAccess guard: always skip property-name position; skip object position only when `accessor === undefined`. JSDOM `let={item, index}` comma-split reassembly added. `ThunkSource` includes `slot-outlet`, `list`, and `classlist` variants; **gained four new structural channels (2026-06-29, nested-structural-emit)** consumed by the recursive `computeBodyThunks` assembler (see below). **Nested structural bindings inside each/recycle/switch bodies LANDED (2026-06-29):** component/each/recycle/switch bindings nested inside an each item body, a recycle item body, a switch branch body, or a switch fallback body are now handled recursively on the Mode-A emit path via `computeBodyThunks` — a recursive assembler that walks a body's own nested structural bindings and recurses into their bodies in turn, so containers can nest inside each other's bodies arbitrarily (no hard-coded depth limit). Includes a `needsSyntheticRoot` auto-wrap fix for bodies that are bare-anchor-only (no real DOM root to attach the synthetic wrapper to). **`<recycle>`-in-`<each>` emit support LANDED (Follow-up A′, 2026-07-02):** the 4th and final Mode-A nesting direction. The prior parse-time throw (`[nv] <recycle> cannot be nested inside an <each> body`) and its accompanying `!isEachBody` binding-push gate in `buildNvSlotContentIR` are removed — `pushRecycledListBinding` now runs unconditionally, symmetric with `pushListBinding`/`pushSwitchBinding` and with `toPendingBundle`'s and `computeBodyThunks`'s pre-existing unconditional handling of `pending.recycles` (those two were already generic; only the binding-push site singled out each-bodies). The now-fully-unused `isEachBody` parameter was removed from `buildNvSlotContentIR`'s signature entirely (dead after the collapse) rather than left as an unused param. `ThunkSource`/pending shape **unchanged** — no new fields, this closes a reachability gap only. Four-direction Mode-A nesting parity reached: component-in-each, each-in-each, switch-in-each, recycle-in-each. **Deviation from the commission's literal "three-back-end parity" wording:** delivered as two-back-end parity (interpreter + Mode-A) instead, matching the `each-in-recycle` precedent — `emitted-mount.ts`'s `recycled-list` case is a pre-existing, unconditional stub (see its own row below), not each-body-specific and out of scope for this landing. **Adjacent gap, still NOT fixed:** component **slot content** (as opposed to each/recycle/switch bodies) still has the old hole-only limitation for nested structural children — the slot-content call sites of `buildNvSlotContentIR` do not get this recursive treatment. Pre-existing, deliberately out of scope for this landing (see `docs/design/design-nested-structural-emit.md`, "Adjacent gap noted, explicitly OUT of scope"). Candidate for its own follow-up with a G1-style nesting matrix. |
| `nv-emitter.ts` | **REAL** | `emitModule(results) → ES module text`. A2 shape + `Name.mount` sugar. **`classlist` case LANDED:** `emitBindingLiteral` case `'classlist'` emits `{ kind: 'classlist', pathIndex, entries: [...] }` with per-toggle `expr: () => (${boolSrc})` — real reactive thunks, not stubs (D-cl-1 closed). Arrow-body paren fix: `((slotProps) => (${bodyLiteral}))` to prevent esbuild block-vs-object parse failure. `emitThunkSource` leaf-only (`LeafThunkSource`); structural kinds handled by `emitBindingLiteral`. Slot thunks fully erased. Imports: only $script-referenced primitives + `mount`. `emitIrLiteral` consumes the four new `ThunkSource` structural channels (2026-06-29, nested-structural-emit) to emit the recursively-assembled each/recycle/switch body thunks produced by `computeBodyThunks` in `nv-parser.ts`. |
...
- **`DeferredSwapBinding` compiler back-end (`emitted-mount.ts`) — STUB.** `wireDeferredSwap`
  (interpreter, semantic ground truth) and both authoring FEs (`.nv` `<switch pending=>`,
  tagged `match(..., pending)`) are real; `emitted-mount.ts`'s `'deferred-swap'` case
  unconditionally throws (mirrors the pre-existing `recycled-list` stub), and
  `nv-emitter.ts`'s codegen switch has no case at all for this kind (falls to its existing
  `default:` throw). Compiling — not interpreting — a `<switch pending=>` file fails loudly
