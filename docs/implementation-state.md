...
| `html-tag.ts` | REAL | Tagged-template front-end (`createHtmlTag(doc)`). Handles `text`, `attr`, `prop` (`.name=`), `event` (`@name=`), `component`, and **`slot-outlet`** (`{slots.name}` holes). Named slot capture: `<slot name="x">` wrappers → `SlotEntry{name, content: TemplateIR}`; reactive holes inside slots captured as slot sub-IR bindings; parent hole indices marked consumed. |
| `nv-parser.ts` | PARTIAL → **REAL for the build path** | Adds `parseNvFileForEmit` + `eraseHandlerExpr` + `computeThunksForTemplate`/`computeThunkSource` + `extractScriptBodySource`/`extractModuleScope`. `parseNvFile` (structural-only, stub thunks) unchanged. `parseNvFileForEmit` returns the real `emit` payload: erased `scriptBody`, index-aligned `bindingThunks` (recursive for conditional + component + **slot-outlet**), `moduleScope`. Named slot capture: `<slot name="x">` → `slotHoleGroups` (index-aligned with `slots[]`). `ThunkSource` includes `{ kind:'slot-outlet'; name }` variant. `computeThunkSource` detects `slots.name` PropertyAccessExpression → slot-outlet ThunkSource. |
