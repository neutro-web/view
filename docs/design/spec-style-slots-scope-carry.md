...
| G3 | scopeHash = `simpleHash(\`${shapeHtml}\0${styleSource}\`)` at both sites (B3 + C1 fix); identical input both back-ends | grep both sites; assert equal hash for same component | sites use `ir.id` or differing input |
| G3' | Two styled components, identical `$style` + identical `shapeHtml` → share scopeHash (correct: same authored style + same shape = same scope); injection dedup merges style+shape identity. Inverse (C1 gate): same `shapeHtml` + different `$style` → distinct scopeHashes (no CSS collision). | mount both; inspect scope hash + injected sheet count | hashes differ for identical inputs, OR dedup wrongly merges components with different `$style` rules |
