...
function assertPolicy(
  ...
  for (const v of verdicts) {
    expect(v.policy, `Expected ${expected}, got ${v.policy} for type '${v.valueTypeString}'`).toBe(
      expected,
    )
  }
  return verdicts
...
test('derived: array return type → FALSE', () => {
  ...
  expect(arrayVerdict, 'derived returning array should be FALSE').toBeTruthy()
})
...
test('derived: object return type → DECLINE', () => {
  ...
  expect(objectVerdict, 'derived returning object should be DECLINE').toBeTruthy()
})
...
test('NO VERDICT: local function named "signal" is not classified', () => {
  ...
  const verdicts = getEqualityVerdicts(`
    ...
    const s = signal(42)
  `)
  expect(verdicts.length, 'local signal() must not produce a verdict').toBe(0)
})
